...
def index(request):
    ...
    num_instances_available = BookInstance.objects.filter(status_exect=2).count()
...
def create(request):
    if request.method == "POST":
        author = Author()
        author.frist_name = request.POST.get("first_name")
...
class LoaneBooksByUserListView(LoginRequiredMixin,generic.ListView):
    model = BookInstance
    template_name = 'catalcg/bookinstance_list_borrowed_user.html'
    paginate_by = 10
